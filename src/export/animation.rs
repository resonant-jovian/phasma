...
pub fn export_animation_frames(
    ...
    if nx > 0 && ny > 0 {
        ...
        let path = frames_dir.join(format!("density_xy_t{:.4}.npy", state.t));
    ...
    if nx > 0 && nz > 0 {
        ...
        let path = frames_dir.join(format!("density_xz_t{:.4}.npy", state.t));
        ndarray_npy::write_npy(&path, &arr).map_err(|e| format!("write npy: {e}"))?;
    }
    ...
    if ny > 0 && nz > 0 {
        ...
        let path = frames_dir.join(format!("density_yz_t{:.4}.npy", state.t));
        ndarray_npy::write_npy(&path, &arr).map_err(|e| format!("write npy: {e}"))?;
    }
    ...
    let csv_path = frames_dir.join("time_series.csv");
