...
impl HelpOverlay {
    ...
    pub fn draw(&self, frame: &mut Frame, area: Rect, theme: &ThemeColors) {
        ...
        let lines = vec![
            ...
            Line::from(vec![
                key("  e         "),
                desc("Export menu (1-9,0,a,z shortcuts)"),
            ]),
            Line::from(vec![key("  Shift+t   "), desc("Cycle theme")]),
            Line::from(vec![key("  Shift+c   "), desc("Cycle colormap (global)")]),
            Line::from(vec![key("  Ctrl+S    "), desc("Save config to file")]),
            Line::from(vec![key("  Ctrl+O    "), desc("Load config (go to Setup)")]),
            Line::from(vec![
                key("  Ctrl+P    "),
                desc("Preset selection (Setup tab)"),
            ]),
            Line::from(vec![
                key("  Ctrl+D    "),
                desc("Reset to defaults (Setup tab)"),
            ]),
            Line::from(vec![key("  :         "), desc("Command palette")]),
            Line::from(vec![key("  /         "), desc("Jump-to-time (shortcut)")]),
            Line::from(vec![
                key("  a         "),
            ...
            Line::from(vec![
                key("  { }       "),
                desc("Jump backward/forward 10 frames"),
            ]),
            Line::from(vec![key("  Home      "), desc("Jump to first frame")]),
            Line::from(vec![key("  End       "), desc("Jump to last frame")]),
            Line::from(vec![key("  Backspace "), desc("Jump to live (exit scrub)")]),
            Line::from(vec![
                key("  < / >     "),
                desc("Decrease/increase playback speed"),
            ]),
            Line::from(vec![
                key("  , / .     "),
                desc("Fine-step backward/forward (playback)"),
            ]),
            Line::from(""),
            section("Setup (F1)"),
            ...
            Line::from(vec![key("  Ctrl+P    "), desc("Preset selection popup")]),
            Line::from(vec![key("  Ctrl+D    "), desc("Reset to defaults")]),
            Line::from(""),
            ...
            Line::from(vec![
                key("  1/2/3     "),
                desc("Spatial dim (x\u{2081}/x\u{2082}/x\u{2083})"),
            ]),
            Line::from(vec![
                key("  4/5/6     "),
                desc("Velocity dim (v\u{2081}/v\u{2082}/v\u{2083})"),
            ]),
            Line::from(vec![key("  +/- scroll"), desc("Zoom in/out")]),
            Line::from(vec![key("  r/0       "), desc("Reset zoom")]),
            Line::from(vec![key("  l         "), desc("Toggle log scale")]),
            ...
            Line::from(vec![key("  </>       "), desc("Slice 4th hidden dim")]),
            Line::from(vec![
                key("  p         "),
            ...
            Line::from(vec![
                key("  s         "),
                desc("Toggle stream-count overlay"),
            ]),
            Line::from(""),
            section("Energy (F5)"),
            Line::from(vec![
            ...
            Line::from(vec![
                key("  1/2/3/4/5 "),
                desc("\u{03c1}(r) / M(r) / \u{03a6}(r) / \u{03c3}(r) / \u{03b2}(r)"),
            ]),
            Line::from(vec![key("  l         "), desc("Toggle log scale")]),
