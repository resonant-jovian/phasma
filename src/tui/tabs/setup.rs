...
impl SetupTab {
    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) -> Option<Action> {
        match key.code {
            KeyCode::Down | KeyCode::Char('j') => {
                if !self.available_configs.is_empty() {
                    self.browser_selected = (self.browser_selected + 1) % self.available_configs.len();
                }
                None
            }
            KeyCode::Up | KeyCode::Char('k') => {
                if !self.available_configs.is_empty() {
                    self.browser_selected = (self.browser_selected + self.available_configs.len() - 1)
            ...
            KeyCode::Enter => {
                if let Some(path) = self.available_configs.get(self.browser_selected).cloned() {
                    ...
                    if self.config_loaded {
                        return Some(Action::ConfigLoaded(self.config_path.clone().unwrap_or(path)));
                    }
                }
                None
            }
            KeyCode::Char('r') if !self.sim_running && self.config_loaded => {
                Some(Action::SimStart)
            }
            _ => None,
        }
    }
    ...
    pub fn update(&mut self, action: &Action) -> Option<Action> {
        match action {
            ...
            Action::SimUpdate(state) => {
                if state.exit_reason.is_some() { self.sim_running = false; }
            }
            _ => {}
        }
        None
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect, theme: &ThemeColors) {
        ...
        let status = if self.sim_running {
            Line::from(vec![
                ...
                Span::styled(" for live view", Style::default().fg(theme.ok)),
            ])
        } else if self.config_loaded {
            Line::from(vec![
                Span::styled(format!("Ready — {mem_estimate:.1} MB est. — press "), Style::default().fg(theme.ok)),
    ...
    fn draw_browser(&mut self, frame: &mut Frame, area: Rect, theme: &ThemeColors) {
        let block = Block::bordered().title(" Configs ").border_style(Style::default().fg(theme.accent));
        ...
        let items: Vec<ListItem> = self.available_configs.iter().enumerate().map(|(i, path)| {
            ...
            if i == self.browser_selected {
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled(label.to_string(), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                ]))
            } else {
    ...
    fn draw_preview(&self, frame: &mut Frame, area: Rect, theme: &ThemeColors) {
        let block = Block::bordered().title(" Config preview ").border_style(Style::default().fg(theme.border));
        let inner = block.inner(area);
        frame.render_widget(block, area);

        if self.config_lines.is_empty() {
            frame.render_widget(
                Paragraph::new("Select a config and press Enter to load,\nor pass --config path/to/run.toml.")
                    .style(Style::default().fg(theme.dim)),
                inner,
            );
            return;
        ...
        let lines: Vec<Line> = self.config_lines.iter().map(|l| {
            let trimmed = l.trim();
            if trimmed.starts_with('#') {
                Line::from(Span::styled(l.clone(), Style::default().fg(theme.dim)))
            } else if trimmed.starts_with('[') {
                Line::from(Span::styled(l.clone(), Style::default().fg(theme.accent).add_modifier(Modifier::BOLD)))
            } else if trimmed.starts_with('✗') {
