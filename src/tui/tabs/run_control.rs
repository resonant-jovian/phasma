...
impl RunControlTab {
    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) -> Option<Action> {
        match key.code {
            KeyCode::Char('p') | KeyCode::Char(' ') => {
                Some(if self.paused { Action::SimResume } else { Action::SimPause })
            }
            KeyCode::Char('s') => Some(Action::SimStop),
            KeyCode::Char('r') => Some(Action::SimRestart),
            KeyCode::Char('1') => { self.log_filter = LogFilter::All; None }
            KeyCode::Char('2') => { self.log_filter = LogFilter::WarnPlus; None }
            KeyCode::Char('3') => { self.log_filter = LogFilter::ErrorOnly; None }
            _ => None,
        }
    }
        ...
        match action {
            Action::SimUpdate(state) => {
                ...
                if self.initial_energy == 0.0 && state.total_energy != 0.0 {
                    ...
                    self.push_log(Level::Info, format!("Sim started: E₀={:.3e}, M₀={:.4}", state.total_energy, state.total_mass));
                }
    ...
    pub fn draw(
        ...
        self.draw_maps(frame, maps_area, theme, colormap);
        self.draw_bottom(frame, bottom_area, theme);
    }

    fn draw_controls(&self, frame: &mut Frame, area: Rect, theme: &ThemeColors) {
        let [prog_area, energy_area] = Layout::vertical([
            Constraint::Length(2),
            Constraint::Length(2),
        ]).areas(area);
        ...
        match &self.sim_state {
            None => {
                frame.render_widget(
                    Paragraph::new(vec![
                        Line::from(Span::styled("No simulation running.", Style::default().fg(theme.dim))),
                        Line::from(vec![
            ...
            Some(state) => {
                ...
                frame.render_widget(
                    Gauge::default()
                        ...
                        .label(format!(
                            ...
                            state.t, state.t_final, state.step, progress * 100.0
                        )),
                    prog_area,
                );
                ...
                let cons_color = if e_drift < 1e-5 { theme.ok }
                    else if e_drift < 1e-3 { theme.warn }
                    else { theme.error };
                ...
                frame.render_widget(
                    Gauge::default()
                        ...
                        .ratio(cons_ratio)
                        .label(format!("|ΔE/E| = {e_drift:.2e}  dt={:.3e}  ρ_max={:.2e}",
                            state.step_wall_ms / 1000.0, state.max_density)),
                    energy_area,
                );
            }
        }
    ...
    fn draw_maps(&self, frame: &mut Frame, area: Rect, theme: &ThemeColors, colormap: Colormap) {
        ...
        match &self.sim_state {
            None => {
                for (a, t) in [(density_area, " ρ(x,y) "), (phase_area, " f(x,vx) ")] {
                    frame.render_widget(
                        Paragraph::new("—").block(Block::bordered().title(t))
            ...
            Some(state) => {
                frame.render_widget(
                    HeatmapWidget::new(&state.density_xy, state.density_nx, state.density_ny, " ρ(x,y) density ")
                        .colormap(colormap),
                    density_area,
                );
                frame.render_widget(
                    HeatmapWidget::new(&state.phase_slice, state.phase_nx, state.phase_nv, " f(x,vx) phase-space ")
                        .colormap(colormap),
                    phase_area,
                );
            }
    ...
    fn draw_bottom(&self, frame: &mut Frame, area: Rect, theme: &ThemeColors) {
        let [left_area, right_area] = Layout::horizontal([
            Constraint::Min(40),
            Constraint::Length(28),
        ]).areas(area);

        let [chart_area, log_area] = Layout::vertical([
        ...
        if self.energy_history.is_empty() {
            ...
            let t_max = self.energy_history.last().map(|(t, _)| *t).unwrap_or(1.0).max(t_min + 0.001);
            let (e_min, e_max) = self.energy_history.iter()
            ...
            .block(Block::bordered().title(" E(t)/E₀ "))
            .x_axis(Axis::default().title("t").style(Style::default().fg(theme.dim))
                .bounds([t_min, t_max])
                .labels(vec![format!("{t_min:.1}"), format!("{:.1}", (t_min+t_max)/2.0), format!("{t_max:.1}")]))
            .y_axis(Axis::default().title("E/E₀").style(Style::default().fg(theme.dim))
                .bounds([e_lo, e_hi])
                .labels(vec![format!("{e_lo:.4}"), format!("{:.4}", (e_lo+e_hi)/2.0), format!("{e_hi:.4}")]));

            frame.render_widget(chart, chart_area);
        }

        // Log stream
        let visible: Vec<_> = self.log_stream.iter()
        ...
        let lines: Vec<Line> = visible.iter().rev().map(|(lvl, msg)| {
            let color = match lvl {
        ...
        match &self.sim_state {
            ...
            Some(state) => {
                ...
                let rows: Vec<SparklineRow> = vec![
                    SparklineRow::new("Energy E", state.total_energy,
                        if init_e != 0.0 { (state.total_energy - init_e)/init_e.abs() } else { 0.0 })
                    ...
                    SparklineRow::new("Virial 2T/|W|", state.virial_ratio, state.virial_ratio - 1.0)
                        .thresholds(0.1, 0.5),
                    SparklineRow::new("Mass M", state.total_mass,
                        if init_m != 0.0 { (state.total_mass - init_m)/init_m.abs() } else { 0.0 })
                        .thresholds(1e-6, 1e-3),
                    SparklineRow::new("Casimir C₂", state.casimir_c2,
                        if init_c != 0.0 { (state.casimir_c2 - init_c)/init_c.abs() } else { 0.0 })
