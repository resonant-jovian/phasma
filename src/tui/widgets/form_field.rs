...
impl FormField {
    ...
    pub fn text_with_unit(label: impl Into<String>, value: impl Into<String>, unit: impl Into<String>) -> Self {
        FormField::TextInput {
            label: label.into(),
            value: value.into(),
            unit: unit.into(),
    ...
    pub fn dropdown(label: impl Into<String>, options: Vec<String>, selected: usize) -> Self {
        FormField::Dropdown { label: label.into(), options, selected, focused: false }
    }

    pub fn toggle(label: impl Into<String>, value: bool) -> Self {
        FormField::Toggle { label: label.into(), value, focused: false }
    }

    pub fn set_focused(&mut self, v: bool) {
        match self {
            FormField::TextInput { focused, editing, .. } => {
                *focused = v;
                if !v { *editing = false; }
            }
    ...
    pub fn value_str(&self) -> String {
        match self {
            ...
            FormField::Dropdown { options, selected, .. } => {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> bool {
        match self {
            FormField::TextInput { value, focused, editing, .. } => {
                ...
                match key.code {
                    ...
                    _ if *editing => {
                        match key.code {
                            KeyCode::Char(c) => { value.push(c); true }
                            KeyCode::Backspace => { value.pop(); true }
                            _ => false,
                        }
                    }
                    _ => false,
            ...
            FormField::Dropdown { options, selected, focused, .. } => {
                if !*focused { return false; }
                match key.code {
            ...
            FormField::Toggle { value, focused, .. } => {
                if !*focused { return false; }
                match key.code {
                    KeyCode::Char(' ') | KeyCode::Enter => { *value = !*value; true }
                    _ => false,
                }
            }
        }
    ...
    pub fn draw(&self, area: Rect, buf: &mut Buffer, theme: &ThemeColors) {
        let (focused, is_editing, has_error) = match self {
            FormField::TextInput { focused, editing, error, .. } => (*focused, *editing, error.is_some()),
            FormField::Dropdown { focused, .. } => (*focused, false, false),
            FormField::Toggle { focused, .. } => (*focused, false, false),
        };

        let line = match self {
            FormField::TextInput { label, value, unit, error, .. } => {
                ...
                if !unit.is_empty() {
                    spans.push(Span::styled(format!(" {unit}"), Style::default().fg(theme.dim)));
                }
                if is_editing {
                    spans.push(Span::styled("  [editing]", Style::default().fg(Color::Yellow).add_modifier(Modifier::DIM)));
                } else if focused {
                    spans.push(Span::styled("  [Enter to edit]", Style::default().fg(theme.dim)));
                }
                if let Some(err) = error {
                    spans.push(Span::styled(format!("  ✗ {err}"), Style::default().fg(theme.error)));
                }
                Line::from(spans)
            }
            FormField::Dropdown { label, options, selected, .. } => {
                let val = options.get(*selected).map(|s| s.as_str()).unwrap_or("—");
                Line::from(vec![
                    Span::styled(format!("{:<16}", label), Style::default().fg(theme.dim)),
                    Span::styled(format!("◄ {val} ►"), Style::default().fg(theme.fg).add_modifier(Modifier::BOLD)),
                ])
            }
            FormField::Toggle { label, value, .. } => {
                let (sym, col) = if *value { ("● ON ", theme.ok) } else { ("○ OFF", theme.dim) };
                Line::from(vec![
                    Span::styled(format!("{:<16}", label), Style::default().fg(theme.dim)),
