...
impl InitConfig {
    fn into_config(self) -> anyhow::Result<Config> {
        let config = Config {
            ...
            _solver: Solver {
                _representation: self.solver.representation,
                _poisson: self.solver.poisson,
                _advection: self.solver.advection,
                _splitting: self.solver.splitting,
            },
            time: Time {
            ...
            _output: Output {
                _interval: Decimal::from_str_exact(&self.output.interval)?,
                _directory: self.output.directory,
                _format: self.output.format,
            },
            exit: Exit {
                energy_tolerance: Decimal::from_scientific(&self.exit.energy_tolerance)?,
                _mass_threshold: Decimal::from_str_exact(&self.exit.mass_threshold)?,
            },
        };
        Ok(config)
    }
