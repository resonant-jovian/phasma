...
fn round_trip(name: &str) {
    ...
    assert_eq!(original.model.model_type, restored.model.model_type,
        "{name}: model_type mismatch after round-trip");
    assert_eq!(original.solver.representation, restored.solver.representation,
        "{name}: representation mismatch after round-trip");
    assert_eq!(original.solver.poisson, restored.solver.poisson,
        "{name}: poisson mismatch after round-trip");
    assert_eq!(original.solver.integrator, restored.solver.integrator,
        "{name}: integrator mismatch after round-trip");
    assert_eq!(original.domain.spatial_resolution, restored.domain.spatial_resolution,
        "{name}: spatial_resolution mismatch after round-trip");
    assert_eq!(original.domain.velocity_resolution, restored.domain.velocity_resolution,
        "{name}: velocity_resolution mismatch after round-trip");
    assert_eq!(original.domain.boundary, restored.domain.boundary,
        "{name}: boundary mismatch after round-trip");

    // Sub-config presence preserved
    assert_eq!(original.model.king.is_some(), restored.model.king.is_some(),
        "{name}: king sub-config presence mismatch");
    assert_eq!(original.model.nfw.is_some(), restored.model.nfw.is_some(),
        "{name}: nfw sub-config presence mismatch");
    assert_eq!(original.model.merger.is_some(), restored.model.merger.is_some(),
        "{name}: merger sub-config presence mismatch");
    assert_eq!(original.model.disk.is_some(), restored.model.disk.is_some(),
        "{name}: disk sub-config presence mismatch");
    assert_eq!(original.model.tidal.is_some(), restored.model.tidal.is_some(),
        "{name}: tidal sub-config presence mismatch");
    assert_eq!(original.model.zeldovich.is_some(), restored.model.zeldovich.is_some(),
        "{name}: zeldovich sub-config presence mismatch");
    assert_eq!(original.model.uniform_perturbation.is_some(), restored.model.uniform_perturbation.is_some(),
        "{name}: uniform_perturbation sub-config presence mismatch");
    assert_eq!(original.solver.ht.is_some(), restored.solver.ht.is_some(),
        "{name}: ht sub-config presence mismatch");
    assert_eq!(original.solver.slar.is_some(), restored.solver.slar.is_some(),
        "{name}: slar sub-config presence mismatch");
    assert_eq!(original.solver.lomac.is_some(), restored.solver.lomac.is_some(),
        "{name}: lomac sub-config presence mismatch");
    assert_eq!(original.solver.multigrid.is_some(), restored.solver.multigrid.is_some(),
        "{name}: multigrid sub-config presence mismatch");
    assert_eq!(original.solver.exponential_sum.is_some(), restored.solver.exponential_sum.is_some(),
...
fn assert_fields(e: &ExpectedFields) {
    ...
    assert_eq!(cfg.model.model_type, e.model_type,
        "{}: model_type", e.name);
    assert_eq!(cfg.solver.representation, e.representation,
        "{}: representation", e.name);
    assert_eq!(cfg.solver.poisson, e.poisson,
        "{}: poisson", e.name);
    assert_eq!(cfg.solver.integrator, e.integrator,
        "{}: integrator", e.name);
    assert_eq!(cfg.domain.spatial_resolution, e.nx,
        "{}: spatial_resolution", e.name);
    assert_eq!(cfg.domain.velocity_resolution, e.nv,
        "{}: velocity_resolution", e.name);
    assert_eq!(cfg.domain.boundary, e.boundary,
        "{}: boundary", e.name);
}
...
field_test!(debug,                  "plummer",               "uniform",              "fft_periodic",        "lie",          4,  4,  "periodic|truncated");
field_test!(disk_bar,               "disk_stability",        "uniform",              "fft_isolated",        "strang",      16, 16,  "isolated|truncated");
field_test!(hernquist,              "hernquist",             "uniform",              "fft_isolated",        "yoshida",     16, 16,  "isolated|truncated");
field_test!(jeans_stable,           "uniform_perturbation",  "uniform",              "fft_periodic",        "strang",       8,  8,  "periodic|open");
field_test!(jeans_unstable,         "uniform_perturbation",  "uniform",              "fft_periodic",        "strang",       8,  8,  "periodic|open");
field_test!(king,                   "king",                  "uniform",              "fft_isolated",        "strang",      16, 16,  "isolated|truncated");
field_test!(merger_equal,           "merger",                "uniform",              "fft_periodic",        "strang",      16, 16,  "periodic|truncated");
field_test!(merger_unequal,         "merger",                "uniform",              "fft_periodic",        "strang",      32, 32,  "periodic|truncated");
field_test!(nfw,                    "nfw",                   "uniform",              "fft_isolated",        "yoshida",     16, 16,  "isolated|truncated");
field_test!(nfw_tree,               "nfw",                   "uniform",              "tree",                "strang",      16, 16,  "isolated|truncated");
field_test!(plummer,                "plummer",               "uniform",              "fft_isolated",        "strang",      16, 16,  "isolated|truncated");
field_test!(plummer_hires,          "plummer",               "uniform",              "fft_isolated",        "yoshida",     32, 32,  "isolated|truncated");
field_test!(plummer_ht,             "plummer",               "hierarchical_tucker",  "fft_isolated",        "strang",      32, 24,  "isolated|truncated");
field_test!(plummer_lomac,          "plummer",               "uniform",              "fft_periodic",        "strang",      16, 16,  "periodic|truncated");
field_test!(plummer_multigrid,      "plummer",               "uniform",              "multigrid",           "strang",      16, 16,  "isolated|truncated");
field_test!(plummer_spectral,       "plummer",               "spectral",             "fft_isolated",        "strang",      16, 16,  "isolated|truncated");
field_test!(plummer_spherical,      "plummer",               "uniform",              "spherical_harmonics", "strang",      16, 16,  "isolated|truncated");
field_test!(plummer_tensor_poisson, "plummer",               "uniform",              "tensor",              "strang",      16, 16,  "isolated|truncated");
field_test!(plummer_tt,             "plummer",               "tensor_train",         "fft_isolated",        "strang",      16, 16,  "isolated|truncated");
field_test!(plummer_unsplit,        "plummer",               "uniform",              "fft_periodic",        "unsplit_rk4",  8,  8,  "periodic|truncated");
field_test!(plummer_yoshida,        "plummer",               "uniform",              "fft_isolated",        "yoshida",     16, 16,  "isolated|truncated");
field_test!(tidal_nfw,              "tidal",                 "uniform",              "fft_isolated",        "strang",      16, 16,  "isolated|truncated");
field_test!(tidal_point,            "tidal",                 "uniform",              "fft_isolated",        "strang",      32, 32,  "isolated|truncated");
field_test!(zeldovich,              "zeldovich",             "uniform",              "fft_periodic",        "strang",      32, 32,  "periodic|truncated");
...
fn nfw_tree_has_nfw_subconfig() {
    ...
    assert!(cfg.model.nfw.is_some(), "nfw_tree.toml must have [model.nfw]");
}

#[test]
fn zeldovich_has_zeldovich_subconfig() {
    let cfg = load_config("zeldovich");
    assert!(cfg.model.zeldovich.is_some(), "zeldovich.toml must have [model.zeldovich]");
}

#[test]
fn merger_equal_has_merger_subconfig() {
    let cfg = load_config("merger_equal");
    assert!(cfg.model.merger.is_some(), "merger_equal.toml must have [model.merger]");
}

#[test]
fn merger_unequal_has_merger_subconfig() {
    let cfg = load_config("merger_unequal");
    assert!(cfg.model.merger.is_some(), "merger_unequal.toml must have [model.merger]");
}

#[test]
fn disk_bar_has_disk_subconfig() {
    let cfg = load_config("disk_bar");
    assert!(cfg.model.disk.is_some(), "disk_bar.toml must have [model.disk]");
}

#[test]
fn tidal_nfw_has_tidal_subconfig() {
    let cfg = load_config("tidal_nfw");
    assert!(cfg.model.tidal.is_some(), "tidal_nfw.toml must have [model.tidal]");
}

#[test]
fn tidal_point_has_tidal_subconfig() {
    let cfg = load_config("tidal_point");
    assert!(cfg.model.tidal.is_some(), "tidal_point.toml must have [model.tidal]");
}

#[test]
fn jeans_stable_has_perturbation_subconfig() {
    let cfg = load_config("jeans_stable");
    assert!(cfg.model.uniform_perturbation.is_some(),
...
fn plummer_ht_has_solver_subconfigs() {
    ...
    assert!(cfg.solver.ht.is_some(), "plummer_ht.toml must have [solver.ht]");
    assert!(cfg.solver.slar.is_some(), "plummer_ht.toml must have [solver.slar]");
    assert!(cfg.solver.lomac.is_some(), "plummer_ht.toml must have [solver.lomac]");
}

#[test]
fn plummer_lomac_has_lomac_subconfig() {
    let cfg = load_config("plummer_lomac");
    assert!(cfg.solver.lomac.is_some(), "plummer_lomac.toml must have [solver.lomac]");
}

#[test]
fn plummer_multigrid_has_multigrid_subconfig() {
    let cfg = load_config("plummer_multigrid");
    assert!(cfg.solver.multigrid.is_some(),
...
fn plummer_tensor_poisson_has_exp_sum_subconfig() {
    ...
    assert!(cfg.solver.exponential_sum.is_some(),
...
fn plummer_spectral_has_ht_subconfig() {
    ...
    assert!(cfg.solver.ht.is_some(), "plummer_spectral.toml must have [solver.ht]");
}

#[test]
fn plummer_tt_has_ht_subconfig() {
    let cfg = load_config("plummer_tt");
    assert!(cfg.solver.ht.is_some(), "plummer_tt.toml must have [solver.ht]");
}
